...
func TestNewPolitenessManager(t *testing.T) {
	...
	pm := NewPolitenessManager(cfg, nil)
	require.NotNil(t, pm)

	assert.Equal(t, cfg.DefaultMinDelay, pm.config.DefaultMinDelay)
	assert.Equal(t, cfg.DefaultDomainConcurrency, pm.config.DefaultDomainConcurrency)
	assert.Equal(t, cfg.GlobalMaxConcurrency, pm.config.GlobalMaxConcurrency)
	assert.NotNil(t, pm.domainStates)
	assert.NotNil(t, pm.robotsCache)
}
...
func TestPolitenessManager_DomainCleanup(t *testing.T) {
	...
	// Check domain exists
	pm.mu.RLock()
	_, exists := pm.domainStates["example.com"]
	pm.mu.RUnlock()
